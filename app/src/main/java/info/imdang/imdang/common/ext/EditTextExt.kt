...
fun EditText.insightTitleValidation(
    maxLength: Int,
    targetTextView: TextView,
    onValidStateChanged: (Boolean) -> Unit
) {
    this.addTextChangedListener(object : TextWatcher {
        ...
        override fun afterTextChanged(s: Editable?) {
            ...
            if (currentText.isNotEmpty() && currentText.first() == ' ') {
                this@insightTitleValidation.setText(currentText.trimStart())
                this@insightTitleValidation.setSelection(this@insightTitleValidation.text.length)
            }

            val currentLength = this@insightTitleValidation.text.length
            onValidStateChanged(currentLength > 0)
        }
    })
}
...
fun EditText.inSightDateValidation(
    errorTextView: TextView,
    statusImageView: ImageView,
    invalidIconResId: Int,
    parentLayout: TextInputLayout,
    ...
    this.addTextChangedListener(object : TextWatcher {
        ...
        override fun afterTextChanged(s: Editable?) {
            ...
            if (s.isNullOrEmpty()) {
                ...
                errorTextView.visibility = View.VISIBLE
                statusImageView.setImageResource(invalidIconResId)
                this@inSightDateValidation.setBackgroundResource(
                    info.imdang.component.R.drawable.sr_white_red_all8
                )
                parentLayout.cursorColor = ColorStateList.valueOf(
                    ContextCompat.getColor(
            ...
            if (s.toString() != formatted) {
                this@inSightDateValidation.setText(formatted)
                this@inSightDateValidation.setSelection(formatted.length)
            }

            val validationResult = validateDate(formatted)
            ...
            when (validationResult) {
                ...
                ValidationResult.INVALID_DATE -> {
                    errorTextView.text =
                        context.getString(
                            info.imdang.component.R.string.insight_date_error_type_form
                        )
                    errorTextView.visibility = View.VISIBLE
                    statusImageView.setImageResource(invalidIconResId)
                    this@inSightDateValidation.setBackgroundResource(
                ...
                ValidationResult.INVALID_YEAR -> {
                    errorTextView.text =
                        context.getString(
                            info.imdang.component.R.string.insight_date_error_type_before
                        )
                    errorTextView.visibility = View.VISIBLE
                    statusImageView.setImageResource(invalidIconResId)
                    this@inSightDateValidation.setBackgroundResource(
        ...
        private fun validateDate(date: String): ValidationResult {
            ...
            if (year < 2020) return ValidationResult.INVALID_YEAR
